...
from acoustic.utils import (Metric, save_checkpoint,
                            load_checkpoint, plot_spectrogram)
...
STEPS = 80000
LOG_INTERVAL = 5
VALIDATION_INTERVAL = 5  # 1000
CHECKPOINT_INTERVAL = 5  # 1000
BACKEND = "nccl"
...
def train(rank, world_size, args):
    ...
    acoustic = AcousticModel(discrete=args.discrete).cuda(rank)
    ...
    n_epochs = STEPS // len(train_dataset) + 1
    start_epoch = global_step // len(train_dataset) + 1
    ...
    for epoch in range(start_epoch, n_epochs + 1):
        ...
        for mels, mels_lengths, units, units_lengths in train_loader:
            pbar.update(1)
            mels, mels_lengths = mels.cuda(rank), mels_lengths.cuda(rank)
            units, units_lengths = units.cuda(rank), units_lengths.cuda(rank)
