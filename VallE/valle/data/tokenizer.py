...
class PypinyinBackend:
    ...
    def __init__(self,
                 backend="initial_finals",
                 punctuation_marks: Union[str,
                                          Pattern] = Punctuation.default_marks(),
                 ) -> None:
                ...
                for n, py in enumerate(pinyin(_text, style=Style.TONE3, neutral_tone_with_five=True)):
                    if all([c in self.punctuation_marks for c in py[0]]):
                        ...
                        if py[0][-1].isalnum():
                            ...
                            if py[0][-1].isdigit():
                                final = (get_finals(
                                    py[0][:-1], strict=False) + py[0][-1])
                            else:
                                final = get_finals(py[0], strict=False)
                            phones.extend(
                                [initial, separator.phone,
                                    final, separator.syllable,])
                        else:
                            assert ValueError
            else:
                raise NotImplementedError
            phonemized.append("".join(phones).rstrip(
                f"{separator.word}{separator.syllable}"))
        return phonemized
...
def tokenize_audio(tokenizer: AudioTokenizer, audip_path: str):
    # Load and pre-process the audio waveform
    wav, sr = torchaudio.load(audip_path)
...
if __name__ == "__main__":
    ...
    # [B,8,T//320]=[4,8,5], encodec has 320 times downsampling
