...
def main():
    ...
    if args.audio_prompts:
        for n, audio_file in enumerate(args.audio_prompts.split("|")):
            encoded_frames = tokenize_audio(
                audio_tokenizer, audio_file)  # [B,8,frames]
            if False:
        ...
        audio_prompts = torch.concat(
            audio_prompts, dim=-1).transpose(2, 1)  # [B,frames,8]
    ...
    for n, text in enumerate(args.text.split("|")):
        ...
        # [B, text_prompt_token_len+target_text_token_lens], [B], these two are {text_prompt target_text}
        ...
        if args.continual:
            ...
            if text_prompts:
                _, enroll_x_lens = text_collater(
                    [tokenize_text(text_tokenizer, text=f"{text_prompts}".strip())])  # [prompt_text_token_lens]
            encoded_frames = model.inference(
                # [B, text_prompt_token_len+target_text_token_lens]
                text_tokens.to(device),
                # [B]=text_prompt_token_len+target_text_token_lens
                text_tokens_lens.to(device),
                # [B, frames,8]
                audio_prompts,
                enroll_x_lens=enroll_x_lens,  # [B]=text_prompt_token_len
                top_k=args.top_k,  # int
                temperature=args.temperature,  # float
            )  # [B, new_generated_franes,8]

        if audio_prompts != []:
            samples = audio_tokenizer.decode(
                [(encoded_frames.transpose(2, 1), None)])  # [B,1,audio_samples]
            # store
            torchaudio.save(f"{args.output_dir}/{n}.wav",
                            samples[0].cpu(), 24000)
        else:  # Transformer
            pass
