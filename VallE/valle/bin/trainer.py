...
def load_checkpoint_if_available(
    ...
    saved_params = load_checkpoint(filename,
                                   model=model,
                                   model_avg=model_avg,
                                   optimizer=optimizer,
                                   scheduler=scheduler,)
...
def train_one_epoch(
    ...
    while True:
        ...
        try:
            with torch.amp.autocast(enabled=enabled, dtype=dtype):
                _, loss, loss_info = compute_loss(params=params,
                                                  ...
                                                  is_training=True)
            # summary stats
            tot_loss = (tot_loss * (1 - 1 / params.reset_interval)
            ...
            if params.batch_idx_train >= params.accumulate_grad_steps:
                if (params.batch_idx_train % params.accumulate_grad_steps == 0):
                    if not params.optimizer_name in ["ScaledAdam", "Eve"]:
        ...
        if params.average_period > 0:
            if (params.batch_idx_train > 0 and params.batch_idx_train % params.average_period == 0):
        ...
        if batch_idx % params.log_interval == 0:
            ...
            cur_grad_scale = (scaler._scale.item() if params.dtype in [
                              "float16", "fp16"] else 1.0)
...
def run(rank, world_size, args):
    ...
    with open(f"{params.exp_dir}/model.txt", "wt", encoding="utf8") as f:
        print(model)
        print(model, file=f)
    ...
    if params.optimizer_name == "ScaledAdam":
        ...
        if params.train_stage:  # !=0
            ...
            parameters_names.append(
                [name_param_pair[0] for name_param_pair in _model.stage_named_parameters(params.train_stage)])
        else:
            parameters_names.append(
                [name_param_pair[0] for name_param_pair in model.named_parameters()])

        optimizer = ScaledAdam(model_parameters,
                               lr=params.base_lr,
                               betas=(0.9, 0.95),
                               clipping_scale=2.0,
        ...
        optimizer = Eve(model_parameters,
                        lr=params.base_lr,
                        betas=(0.9, 0.95),
                        target_rms=0.1,)
    ...
    # Recover optimizer and scheduler states
    if checkpoints and "optimizer" in checkpoints:
        logging.info("Loading optimizer state dict")
        optimizer.load_state_dict(checkpoints["optimizer"])

    if checkpoints and "scheduler" in checkpoints and not checkpoints["scheduler"] is None:
    ...
    if checkpoints and params.start_batch > 0 and "sampler" in checkpoints:
    ...
    for criterion, cuts in batches.items():
        ...
        try:
            ...
            if "CUDA out of memory" in str(e):
                logging.error("Your GPU ran out of memory with the current "
