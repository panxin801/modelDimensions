...
def get_args():
    ...
    parser.add_argument("--src-dir", type=Path,
                        default=Path("data/manifests"), help="Path to the manifest files")
    parser.add_argument("--output-dir", type=Path,
                        default=Path("data/tokenized"), help="Path to the tokenized files")
    parser.add_argument("--text-extractor", type=str,
                        default="espeak", help="espeak or pypinyin or pypinyin_initials_finals")
    parser.add_argument("--audio-extractor", type=str,
                        default="Encodec", help="Encodec or Fbank")
    parser.add_argument("--dataset-parts", type=str,
                        default="dev-clean test-clean", help="Space separated dataset parts")
    parser.add_argument("--prefix", type=str,
                        default="libritts", help="prefix of the manifest file")
    parser.add_argument("--suffix", type=str,
                        default="jsonl.gz", help="suffix of the manifest file")
    parser.add_argument("--batch-duration", type=float,
                        default=400.0, help="The maximum number of audio seconds in a batch. Determines batch size dynamically.")
...
def main():
    ...
    num_jobs = min(os.cpu_count() - 2, 32)
    logging.info(f"dataset_parts: {dataset_parts} manifests {len(manifests)}")
    ...
    with get_executor() as ex:
        for partition, m in manifests.items():
            ...
            if args.audio_extractor:
                ...
                if args.prefix.lower() in ["ljspeech", "aishell", "baker_zh", "baker"]:
            ...
            if args.text_extractor:
                if (args.prefix.lower() in ["baker", "baker_zh"] and args.text_extractor == "labeled_pinyin"):
