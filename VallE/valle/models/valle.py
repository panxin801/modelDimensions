...
from ..modules.embedding import (SinePositionalEmbedding,
                                 TokenEmbedding)
from ..modules.transformer import (AdaptiveLayerNorm,
                                   LayerNorm,
                                   TransformerDecoderLayer,
                                   TransformerEncoder,
                                   TransformerEncoderLayer,)
from .visualizer import visualize
from .macros import (NUM_TEXT_TOKENS,
                     NUM_AUDIO_TOKENS)
...
class VALLF(nn.Module):
    ...
    def __init__(self,
        ...
        self.ar_text_position = SinePositionalEmbedding(d_model,
                                                        ...
                                                        scale=False,
                                                        alpha=True)
        self.ar_audio_position = SinePositionalEmbedding(d_model,
                                                         dropout=0.1,
                                                         scale=False,
                                                         alpha=True)
        ...
        if num_quantizers > 1:
            ...
            self.nar_text_position = SinePositionalEmbedding(nar_d_model,
                                                             dropout=0.0,
                                                             scale=False,
                                                             alpha=False)
            self.nar_audio_position = SinePositionalEmbedding(nar_d_model,
                                                              dropout=0.0,
                                                              scale=False,
                                                              alpha=False)
            ...
            self.nar_predict_layers = nn.ModuleList([nn.Linear(
                nar_d_model, NUM_AUDIO_TOKENS, bias=False) for i in range(num_quantizers - 1)])
        ...
        if self.prefix_mode == 0:
            ...
            if self.prefix_mode == 2:
                ...
                for b in range(codes.shape[0]):
                    ...
                    y_prompts_codes.append(torch.clone(
                        codes[b, start:start + prefix_len]))
                    codes[b, start:start + prefix_len,
                          nar_stage] = NUM_AUDIO_TOKENS
    ...
    def forward(self,
        ...
        if isinstance(y, PromptedFeatures):
            ...
            y_prompts_codes = y_prompts_codes.type(torch.long)
        ...
        if train_stage in [0, 2]:
            ...
            metrics["NarTop10Accuracy"] = (self.nar_accuracy_metric(
                F.pad(logits.detach(), (0, 0, 0, 1, 0, 0),
                      value=logits.min().cpu().item()),
                targets).item() * total_length)
    ...
    def inference(self,
        ...
        while True:
            ...
            tgt_mask = torch.triu(torch.ones(y.shape[1], y.shape[1],
                                             device=y.device, dtype=torch.bool),
                                  diagonal=1,)
            y_dec, _ = self.ar_decoder((y_pos, None),
                                       x,
                                       tgt_mask=tgt_mask,
            ...
            if (torch.argmax(logits, dim=-1)[0] == NUM_AUDIO_TOKENS or
                samples[0, 0] == NUM_AUDIO_TOKENS or
                    (y.shape[1] - prefix_len) > x_lens.max() * 16):
        ...
        if self.prefix_mode in [2, 4]:  # Exclude enrolled_phonemes
            ...
            text = torch.concat(
                [text[:, :1], text[:, enrolled_len - 1:]], dim=1)
        ...
        for i, (predict_layer, embedding_layer) in enumerate(
            ...
            y_pos = self.nar_audio_prenet(y_emb)
            y_pos = self.nar_audio_position(y_pos)
            y_dec, _ = self.nar_decoder((y_pos, self.nar_stage_embeddings[i].weight),
                                        x,
                                        tgt_mask=None,
                                        memory_mask=None,
                                        memory_key_padding_mask=x_mask)
...
class VALLE(VALLF):
    ...
    def inference(self,
        ...
        while True:
            ...
            if (torch.argmax(logits, dim=-1)[0] == NUM_AUDIO_TOKENS or
                samples[0, 0] == NUM_AUDIO_TOKENS or
                    (y.shape[1] - prompts.shape[1]) > x_lens.max() * 16):
                if prompts.shape[1] == y.shape[1]:
                    raise SyntaxError(
                        "well trained model shouldn't reach here.")
