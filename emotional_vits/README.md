...
这个方案的emotional 建模是通过基于wav2vec2微调的一个模型完成的emotional embedding的推理，这个wav2vec2微调的模型本身还有一个classifier应该是用于训练的时候做分类的，但是在推理的时候，使用的是wav2vec2出来的`hidden_states`。然后每个音频都有一个emo embedding， 把这个和文本编码特征放到一起。这个流程就是上边的图那个过程。

但我自己有个问题，就是不同的人对于平静的语气是不一样的。使用这个embedding会不会有影响。

除了这用这个wav2vec2 finetune emotional 的还有一个是`nvidia/mellotron` 这个包。里边使用了`gst-tacotron2`的`gst`模块。