...
class SynthesizerEval(nn.Module):
    ...
    def infer_pause(self, x, x_lengths, bert, pause_mask, pause_value, sid=None, noise_scale=1, length_scale=1, max_len=None):
        """ Args: 
                pause_mask: 停顿的掩膜，停顿的位置是0,不停顿位置是1
                pause_value: 停顿的填充值，停顿的位置是停顿的值时长,不停顿的位置是0
        """
        x, m_p, logs_p, x_mask = self.enc_p(x, x_lengths, bert)
        if self.n_speakers > 0:
            g = self.emb_g(sid).unsqueeze(-1)  # [b, h, 1]
        ...
        w = torch.exp(logw) * x_mask * length_scale
        w_ceil = torch.ceil(w + 0.35)
        w_ceil = w_ceil * pause_mask + pause_value  # w_ceil中停顿位置摸成0,然后把pause_value加上去。
        y_lengths = torch.clamp_min(torch.sum(w_ceil, [1, 2]), 1).long()
        y_mask = torch.unsqueeze(commons.sequence_mask(y_lengths, None), 1).to(
            x_mask.dtype
        )
    ...
    def infer_stream(self, x, x_lengths, bert, sid=None, noise_scale=1, length_scale=1):
        ...
        hop_length = 256  # bert_vits.json， 窗移点数
        hop_frame = 9  # 窗移帧数
        hop_sample = hop_frame * hop_length  # 窗移点数
        stream_chunk = 100  # chunk 帧数
        stream_index = 0  # 开始帧
        stream_out_wav = []

        while (stream_index + stream_chunk < len_z):
            if (stream_index == 0):  # start frame
                cut_s = stream_index  # 开始帧第一次是0
                cut_s_wav = 0  # 开始音频点第一次是0
            else:
                cut_s = stream_index - hop_frame  # 左侧有hop_frame帧的重和，使结果接缝连续
                cut_s_wav = hop_sample  # 因为重叠帧的存在，新数据从hop_sample开始
            ...
            if (stream_index + stream_chunk > len_z - hop_frame):  # end frame
                ...
                cut_e = stream_index + stream_chunk + \
                    hop_frame  # 终止帧，右侧计算多hop_frame帧，然后截取，接缝就自然。
                cut_e_wav = -1 * hop_sample  # wav停止点需要去掉hop_sample
