...
class Upsample1D(nn.Module):
    ...
    def __init__(self, channels: int, out_channels: int, stride: int = 2):
        ...
        self.conv = nn.Conv1d(self.channels, self.out_channels,
                              stride * 2 + 1, stride=1, padding=0)

    def forward(self, inputs: torch.Tensor, input_lengths: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        outputs = F.interpolate(
            inputs, scale_factor=float(self.stride), mode="nearest")
...
class UpsampleConformerEncoder(torch.nn.Module):
    ...
    def __init__(
        ...
        self.pre_lookahead_layer = PreLookaheadLayer(
            in_channels=512, channels=512, pre_lookahead_len=3)
    ...
    def forward(
        ...
        if context.size(1) != 0:
            ...
            context, _, _ = self.embed(
                context, context_masks, offset=xs.size(1))
