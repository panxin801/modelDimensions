...
class CausalConv1d(nn.Conv1d):
    ...
    def forward(self,
                x: torch.Tensor,
                cache: torch.Tensor = torch.zeros(0, 0, 0)) -> Tuple[torch.Tensor,]:
        input_timestep = x.size(2)
        if cache.size(2) == 0:
            cache = torch.zeros(x.size(0), x.size(
                1), self.causal_padding).to(x)
        assert cache.size(2) == self.causal_padding
        if self.causal_type == "left":
            x = torch.cat([cache, x], dim=2)
        else:
            x = torch.cat([x, cache], dim=2)
        x = super().forward(x)
        assert x.size(2) == input_timestep
...
class CausalConv1dUpsample(torch.nn.Conv1d):
    ...
    def forward(self,
        ...
        x = super().forward(x)
        assert input_timestep == x.size(2)
        return x
