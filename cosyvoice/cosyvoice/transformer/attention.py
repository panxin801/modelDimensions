...
class MultiHeadedAttention(nn.Module):
    ...
    def forward_attention(self, value: torch.Tensor,
        ...
        if mask.size(2) > 0:  # time2>0
            ...
            attn = F.softmax(scores, dim=-1).masked_fill(mask,
                                                         0.0)  # (batch, head, time1, time2)
        # NOTE(xcsong): When will `if mask.size(2) > 0` be False?
        #   1. onnx(16/-1, -1/-1, 16/0)
        #   2. jit (16/-1, -1/-1, 16/0, 16/4)
        else:
            attn = F.softmax(scores, dim=-1)  # (batch, head, time1, time2)
...
class RelPositionMultiHeadedAttention(MultiHeadedAttention):
    ...
    def rel_shift(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_padded = x_padded.view(x.size(0), x.size(1),
                                 x.size(3) + 1, x.size(2))
