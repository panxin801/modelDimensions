...
def init_dataset_and_dataloader(args, configs, gan, dpo):
    ...
    train_dataset = Dataset(args.train_data,
                            ...
                            shuffle=True,
                            partition=True)
    cv_dataset = Dataset(args.cv_data,
                         data_pipeline=data_pipeline,
                         mode="train",
                         gan=gan,
                         dpo=dpo,
    ...
    # do not use persistent_workers=True,
    # as whisper tokenizer opens tiktoken file each time when the for loop starts
...
def check_modify_and_save_config(args, configs):
    if args.train_engine == "torch_ddp":
        ...
        configs["train_conf"]["save_per_step"] = int(
            configs["train_conf"]["save_per_step"] * configs["train_conf"]["accum_grad"] / ds_configs["gradient_accumulation_steps"])
...
def init_optimizer_and_scheduler(args, configs, model, gan):
    if gan is False:
        if configs["train_conf"]["optim"] == "adam":
            ...
            raise ValueError(f"unknown optimizer: {configs['train_conf']}")
        ...
        if configs["train_conf"]["scheduler"] == "warmuplr":
            ...
            raise ValueError(f"unknown scheduler: {configs['train_conf']}")
        ...
        optimizer_d, optimizer_g = None, None
    else:
        ...
        if configs["train_conf"]["optim_d"] == "adam":
            optimizer_d = optim.Adam(
                model.module.discriminator.parameters(), **configs["train_conf"]["optim_conf"])
...
def save_model(model, model_name, info_dict):
    ...
    if rank == 0:
        ...
        info_dict["save_time"] = datetime.datetime.now().strftime(
            "%d/%m/%Y %H:%M:%S")
        with open(info_path, "w") as fout:
            data = yaml.dump(info_dict)
            fout.write(data)
        logging.info(
            f"[Rank {rank}] Checkpoint: save to checkpoint {save_model_path}")
...
def cosyvoice_join(group_join, info_dict):
    ...
    if info_dict["batch_idx"] != 0:
        ...
        try:
            ...
            logging.info(
                f"Detected uneven workload distribution: {e}\n" + f"Break current worker to manually join all workers, " + f"world_size {world_size}, current rank {rank}, current local_rank {local_rank}\n")
            return True
    else:
        return False


def batch_forward(model,
                  batch,
                  scaler,
                  info_dict,
                  ref_model=None,
    ...
    if info_dict["train_engine"] == "torch_ddp":
        autocast = torch.autocast(
            device_type="cuda", dtype=dtype, enabled=scaler is not None)
    else:
        autocast = torch.autocast(
            device_type="cuda", dtype=dtype, enabled=True, cache_enabled=False)

    with autocast:
        info_dict["loss_dict"] = model(batch, device)
        if not ref_model is None and not dpo_loss is None:
            chosen_logps = info_dict["loss_dict"]["chosen_logpsn"]
            rejected_logps = info_dict["loss_dict"]["rejected_logps"]
            sft_loss = info_dict["loss_dict"]["loss"]
            with torch.no_grad():
                ref_loss_dict = ref_model(batch, device)
            reference_chose_logps = ref_loss_dict["chosen_logps"]
            reference_rejected_logps = ref_loss_dict["rejected_logps"]
            preference_loss, chosen_reward, reject_reward = dpo_loss(chosen_logps,
                                                                     rejected_logps,
                                                                     reference_chose_logps,
            ...
            info_dict["loss_dict"]["chose_reward"] = chosen_reward.mean()
            info_dict["loss_dict"]["reject_reward"] = reject_reward.mean()
    return info_dict


def batch_backward(model,
                   scaler,
                   info_dict):
    ...
    if info_dict["train_engine"] == "deepspeed":
        ...
        if not scaler is None:
            grad_norm = clip_grad_norm_(
                model.parameters(), info_dict["grad_clip"])
