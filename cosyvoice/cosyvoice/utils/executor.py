...
class Executor:
            ...
            for batch_idx, batch_dict in enumerate(train_data_loader):
                ...
                with context():
                    info_dict = batch_forward(
                        model, batch_dict, scaler, info_dict, ref_model=self.ref_model, dpo_loss=self.dpo_loss)
                ...
                if info_dict["save_per_step"] > 0 and (self.step + 1) % info_dict["save_per_step"] == 0 and (batch_idx + 1) % info_dict["accum_grad"] == 0:
                    dist.barrier()
                    self.cv(model, cv_data_loader, writer,
                            info_dict, on_batch_end=False)
                    model.train()
        ...
        with model_context():
            for batch_idx, batch_dict in enumerate(train_data_loader):
                ...
                with context():
                    ...
                    info_dict = batch_forward(
                        model, batch_dict, scaler, info_dict)
                    info_dict = batch_backward(model, scaler, info_dict)
                info_dict = update_parameter_and_lr(
    ...
    def cv(self,
        ...
        logging.info(
            f"Epoch {self.epoch} Step {self.step+1} on_batch_end {on_batch_end} CV rank {self.rank}")
        model.eval()
