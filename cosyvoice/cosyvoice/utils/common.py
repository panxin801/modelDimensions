...
def ras_sampling(weighted_scores, decoded_tokens, sampling, top_p=0.8, top_k=25, win_size=10, tau_r=0.1):
    ...
    rep_num = (torch.tensor(
        decoded_tokens[-win_size:]).to(weighted_scores.device) == top_ids).sum().item()
...
def nucleus_sampling(weighted_scores, top_p=0.8, top_k=25):
    prob, indices = [], []
    cum_prob = 0.0
    sorted_value, sorted_idx = weighted_scores.softmax(
        dim=0).sort(descending=True, stable=True)
    ...
    indices = torch.tensor(indices, dtype=torch.long).to(
        weighted_scores.device)
    top_ids = indices[prob.multinomial(1, replacement=True)].item()
    return top_ids


def random_sampling(weighted_scores, decoded_tokens, sampling):
    top_ids = weighted_scores.softmax(
        dim=0).multinomial(1, replacement=True).item()
    return top_ids
...
class TrtContextWrapper:
    def __init__(self, trt_engine, trt_concurrent=1, device="cuda:0"):
        ...
        for _ in range(trt_concurrent):
            ...
            trt_stream = torch.cuda.stream(torch.cuda.Stream(device=device))
            assert trt_context is not None, f"Failed to create trt context, maybe not enough CUDA memory, try reduce current trt concurrent {trt_concurrent}"
