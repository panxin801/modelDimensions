...
class TextEmbedding(nn.Module):
    def __init__(self,
        ...
        self.text_embed = nn.Embedding(
            text_num_embeds + 1, text_dim)  # use 0 as filler token

        if conv_layers > 0:
            self.extra_modeling = True
            self.precompute_max_pos = 4096  # ~44s of 24khz audio
            self.register_buffer("freqs_cls", precompute_freqs_cis(
                text_dim, self.precompute_max_pos), persistent=False)
    ...
    def forward(self,
                text: int["b nt"],
                seq_len,
                drop_text=False):  # noqa: F722
        batch, text_len = text.size(0), text.size(1)
        ...
        if self.extra_modeling:
            ...
            pos_id = get_pos_embed_indices(
                batch_start, seq_len, max_pos=self.precompute_max_pos)
            text_pos_embed = self.freqs_cls[pos_id]
...
class InputEmbedding(nn.Module):
    ...
    def forward(self,
        ...
        if self.spk_dim > 0:
            spks = repeat(spks, "b c -> b t c", t=x.size(1))
            to_cat.append(spks)

        x = self.proj(torch.cat(to_cat, -1))
    ...
    def __init__(self,
        ...
        self.transformer_blocks = nn.ModuleList(
            [DiTBlock(dim=dim, heads=heads, dim_head=dim_head,
                      ff_mult=ff_mult, dropout=dropout) for _ in range(depth)])
    ...
    def forward(self,
        ...
        if streaming is True:
            attn_mask = add_optional_chunk_mask(
                x, mask.bool(), False, False, 0, self.static_chunk_size, -1).unsqueeze(1)
        else:
            attn_mask = add_optional_chunk_mask(
                x, mask.bool(), False, False, 0, 0, -1).repeat(1, x.size(1), 1).unsqueeze(1)

        for block in self.transformer_blocks:
            x = block(x, t, mask=attn_mask.bool(), rope=rope)

        if self.long_skip_connection is not None:
            x = self.long_skip_connection(torch.cat((x, residual), -1))
