...
from cosyvoice.transformer.convolution import (
    CausalConv1d, CausalConv1dDownSample, CausalConv1dUpsample)
...
class SineGen(torch.nn.Module):
    ...
    def forward(self, f0):
        ...
        phase_vec = u_dist.sample(sample_shape=(
            f0.size(0), self.harmonic_num + 1, 1)).to(F_mat.device)
        phase_vec[:, 0, :] = 0
...
class SineGen2(torch.nn.Module):
    ...
    def _f02sine(self, f0_values):
        ...
        if self.training is False and self.causal is True:
            rad_values[:, 0, :] = rad_values[:, 0, :] + \
                self.rand_ini.to(rad_values.device)
        else:
            rand_ini = torch.rand(
        ...
        if not self.flag_for_pulse:
            ...
            phase = F.interpolate(phase.transpose(1, 2) * self.upsample_scale,
                                  ...
                                  mode="nearest" if self.causal is True else 'linear').transpose(1, 2)
    ...
    def forward(self, f0):
        ...
        if self.training is False and self.causal is True:
            noise = noise_amp * \
                self.sine_waves[:, :sine_waves.shape[1]].to(sine_waves.device)
        else:
            noise = noise_amp * torch.randn_like(sine_waves)
...
class HiFTGenerator(nn.Module):
    ...
    def __init__(
        ...
        for i, (u, k, d) in enumerate(zip(downsample_cum_rates[::-1], source_resblock_kernel_sizes, source_resblock_dilation_sizes)):
            if u == 1:
                self.source_downs.append(
                    nn.Conv1d(istft_params["n_fft"] + 2,
                              base_channels // (2 ** (i + 1)), 1, 1)
                )
            else:
                self.source_downs.append(
                    nn.Conv1d(istft_params["n_fft"] + 2, base_channels //
                              (2 ** (i + 1)), u * 2, u, padding=(u // 2))
                )
        ...
        self.conv_post = weight_norm(
            nn.Conv1d(ch, istft_params["n_fft"] + 2, 7, 1, padding=3))
        self.ups.apply(init_weights)
        self.conv_post.apply(init_weights)
        self.reflection_pad = nn.ReflectionPad1d((1, 0))
        self.stft_window = torch.from_numpy(get_window(
            "hann", istft_params["n_fft"], fftbins=True).astype(np.float32))
    ...
    def _stft(self, x):
        spec = torch.stft(
            x,
            self.istft_params["n_fft"], self.istft_params["hop_len"], self.istft_params["n_fft"], window=self.stft_window.to(
                x.device),
            return_complex=True)
    ...
    def _istft(self, magnitude, phase):
        ...
        inverse_transform = torch.istft(torch.complex(real, img),
...
class CausalHiFTGenerator(HiFTGenerator):
    ...
    def __init__(
        ...
        for i, (u, k, d) in enumerate(zip(downsample_cum_rates[::-1], source_resblock_kernel_sizes, source_resblock_dilation_sizes)):
            if u == 1:
                self.source_downs.append(
                    CausalConv1d(
                        istft_params["n_fft"] + 2, base_channels // (2 ** (i + 1)), 1, 1, causal_type='left')
                )
            else:
                self.source_downs.append(
                    CausalConv1dDownSample(
                        istft_params["n_fft"] + 2, base_channels // (2 ** (i + 1)), u * 2, u)
                )
    ...
    def decode(self, x: torch.Tensor, s: torch.Tensor = torch.zeros(1, 1, 0), finalize: bool = True) -> torch.Tensor:
        ...
        if finalize is True:
            ...
            x = self.conv_pre(x[:, :, :-self.conv_pre_look_right],
        ...
        if finalize is False:
            x = x[:, :-int(np.prod(self.upsample_rates) *
                           self.istft_params['hop_len'])]
        x = torch.clamp(x, -self.audio_limit, self.audio_limit)
        return x
    ...
    def inference(self, speech_feat: torch.Tensor, finalize: bool = True) -> torch.Tensor:
        ...
        f0 = self.f0_predictor(
            speech_feat.cpu(), finalize=finalize).to(speech_feat)
        # f0->source
        s = self.f0_upsamp(f0[:, None]).transpose(1, 2)  # bs,n,t
        s, _, _ = self.m_source(s)
        s = s.transpose(1, 2)
        if finalize is True:
            generated_speech = self.decode(
                x=speech_feat, s=s, finalize=finalize)
        else:
            generated_speech = self.decode(
...
if __name__ == '__main__':
    ...
    for i in range(0, max_len, chunk_size):
        ...
        pred_chunk, _ = model.inference(
            mel[:, :, : i + chunk_size + context_size], finalize=finalize)
        pred_chunk = pred_chunk[:, i * 480:]
        print((pred_gt[:, i * 480:i * 480 + pred_chunk.shape[1]
                       ] - pred_chunk).abs().max().item())
