...
from cosyvoice.utils.onnx import (
    SpeechTokenExtractor, onnx_path, online_feature)
...
class TransformerLM(nn.Module):
    ...
    def inference(self,
        ...
        # instruct: 1+0+T_text+1+0, embedding is None, prompt_speech_token is none.
...
class Qwen2Encoder(nn.Module):
    ...
    def forward(self, xs: torch.Tensor,
        ...
        outs = self.model(inputs_embeds=xs,
...
class Qwen2LM(TransformerLM):
    def __init__(self,
        ...
        self.criterion_ce = LabelSmoothingLoss(size=speech_token_size + 3,
    ...
    def prepare_lm_input_target(self,
        ...
        if instruct_token is not None and instruct_token_emb is not None and instruct_token_len is not None:
            ...
            instruct_token = [torch.empty(0).to(
                text_token[0])] * len(text_token)
            instruct_token_emb = [torch.empty(0, 896).to(
                text_token_emb[0])] * len(text_token)
            instruct_token_len = torch.zeros(
                len(text_token)).to(text_token_len)
            for i in range(len(text_token)):
                # bistream sequence
                if random.random() < 0.5 and speech_token_len[i] / text_token_len[i] > self.mix_ratio[1] / self.mix_ratio[0]:
                    this_lm_target, this_lm_input = [
                        IGNORE_ID], [sos_emb.squeeze(0)]
                    this_lm_target += [IGNORE_ID] * instruct_token_len[i]
                    this_lm_target.append(instruct_token_emb[i])
                    for j in range(((text_token_len[i] + 1) / self.mix_ratio[0]).ceil().int().item()):
                        this_text_token = text_token[i][j * self.mix_ratio[0]:(
                            j + 1) * self.mix_ratio[0]].tolist()
                        this_speech_token = speech_token[i][j * self.mix_ratio[1]:(
                            j + 1) * self.mix_ratio[1]].tolist()
                        if len(this_text_token) == self.mix_ratio[0]:
                            ...
                            this_lm_input.append(
                                text_token_emb[i][j * self.mix_ratio[0]:(j + 1) * self.mix_ratio[0]])
                            this_lm_input.append(
                                speech_token_emb[i][j * self.mix_ratio[1]:(j + 1) * self.mix_ratio[1]])
                        else:
                            this_lm_target += [-1] * len(this_text_token)
                            this_lm_target += speech_token[i][j *
                                                              self.mix_ratio[1]:].tolist()
                            this_lm_target.append(self.eos_token)
                            this_lm_input.append(
                                text_token_emb[i][j * self.mix_ratio[0]:])
                            this_lm_input.append(task_id_emb.squeeze(0))
                            this_lm_input.append(
                                speech_token_emb[i][j * self.mix_ratio[1]:])
                    this_lm_target, this_lm_input = torch.tensor(
                        this_lm_target), torch.concat(this_lm_input, dim=0)
                # unistream sequence
                else:
                    this_lm_target = torch.tensor(
                        [IGNORE_ID] * (1 + instruct_token_len[i] + text_token_len[i]) + speech_token[i].tolist() + [self.eos_token])
                    this_lm_input = torch.concat([sos_emb.squeeze(
                        0), instruct_token_emb[i], text_token_emb[i], task_id_emb.squeeze(0), speech_token_emb[i]], 0)
                lm_target.append(this_lm_target)
                lm_input.append(this_lm_input)
            lm_input_len = torch.tensor([i.size(0)
                                        for i in lm_input], dtype=torch.long)
            lm_input = pad_sequence(
                lm_input, batch_first=True, padding_value=IGNORE_ID)
            lm_target = pad_sequence(
    ...
    def forward(self,
        ...
        if self.__class__.__name__ == "CosyVoice3LM":
            ...
            instruct_token_emb = self.llm.model.model.embed_tokens(
                instruct_token)
            lm_target, lm_input, lm_input_len = self.prepare_lm_input_target(sos_emb,
                                                                             text_token,
                                                                             text_token_emb,
                                                                             text_token_len,
                                                                             task_id_emb,
        ...
        acc = th_accuracy(
            logits.view(-1, self.llm_decoder.out_features), lm_target, ignore_label=IGNORE_ID)
        return {"loss": loss, "acc": acc}

    def forward_dpo(self,
                    batch: dict,
                    device: torch.device) -> Dict[str, Optional[torch.Tensor]]:
        text_token = batch["text_token"].to(device)
        text_token_len = batch["text_token_len"].to(device)
        ...
        speech_token_combined_len = torch.concat(
            [speech_token_len, reject_speech_token_len], 0)
        speech_token_combined_emb = self.speech_embedding(
        ...
        lm_target, lm_input, lm_input_len = self.prepare_lm_input_target(sos_emb,
                                                                         text_token.repeat(
                                                                             2, 1),
                                                                         text_token_emb.repeat(
                                                                             2, 1, 1),
                                                                         text_token_len.repeat(
                                                                             2),
                                                                         task_id_emb,
                                                                         speech_token_combined,
        ...
        lm_output, lm_output_mask = self.llm(lm_input, lm_input_len.to(device))
        logits = self.llm_decoder(lm_output)
        chosen_logits = logits[:text_token.size(0)]
        rejected_logits = logits[text_token.size(0):]
        chosen_lm_target = lm_target[:text_token.size(0)]
        rejected_lm_target = lm_target[text_token.size(0):]
        loss = self.criterion_ce(chosen_logits, chosen_lm_target.to(device))
        acc = th_accuracy(chosen_logits.view(-1, self.speech_token_size + 3),
                          chosen_lm_target, ignore_label=IGNORE_ID)

        # 5. calculate dpo logits
        chosen_lm_mask = chosen_lm_target == IGNORE_ID
        rejected_lm_mask = rejected_lm_target == IGNORE_ID
        chosen_logps = torch.gather(chosen_logits.log_softmax(
            dim=-1), dim=2, index=chosen_lm_target.masked_fill(chosen_lm_mask, 0).unsqueeze(dim=-1)).squeeze(dim=-1)
        rejected_logps = torch.gather(rejected_logits.log_softmax(
            dim=-1), dim=2, index=rejected_lm_target.masked_fill(rejected_lm_mask, 0).unsqueeze(dim=-1)).squeeze(dim=-1)
        chosen_logps = (chosen_logps * chosen_lm_mask).sum(dim=-
                                                           1) / chosen_lm_mask.sum(dim=-1)
        rejected_logps = (
            rejected_logps * rejected_lm_mask).sum(dim=-1) / rejected_lm_mask.sum(dim=-1)
    ...
    def inference_wrapper(self,
        ...
        if hasattr(self, "vllm"):
            ...
            sampling_params = SamplingParams(top_k=sampling,
                                             ...
                                             max_tokens=max_len)
            with self.lock:
                self.vllm.add_request(uuid, {"prompt_embeds": lm_input.squeeze(
                    0).to(torch.bfloat16).to(lm_input.device)}, sampling_params)
                ...
                with self.lock:
                    if self.vllm_output_queue[uuid].empty() is True:
                        ...
                        for request_output in request_outputs:
                            top_ids = list(
                                request_output.outputs[0].token_ids)[-1]
    ...
    def inference_bistream(
            self,
            text: Generator,
            prompt_text: torch.Tensor,  # [B, T_prompt_text_token=15]
            prompt_text_len: torch.Tensor,
            prompt_speech_token: torch.Tensor,  # [B, T_prompt_speec_token=87]
            prompt_speech_token_len: torch.Tensor,
            embedding: torch.Tensor,  # spk embedding [B, 192]
            sampling: int = 25,
            max_token_text_ratio: float = 20,
            min_token_text_ratio: float = 2,) -> Generator[torch.Tensor, None, None]:
        ...
        if prompt_speech_token_len != 0:
            prompt_speech_token_emb = self.speech_embedding(
                prompt_speech_token)  # [B, T_prompt_speec_token, 896]
        else:
            prompt_speech_token_emb = torch.zeros(
                1, 0, self.llm_input_size, dtype=prompt_text.dtype, device=device)
        lm_input = torch.concat([sos_emb], 1)  # only sos symbol
        ...
        if self.__class__.__name__ == "CosyVoice3LM":
            ...
            lm_input = torch.concat(
                [lm_input, self.llm.model.model.embed_tokens(prompt_text[:, :eop_index + 1])], 1)
            prompt_text = prompt_text[:, eop_index + 1:]
        text_cache = self.llm.model.model.embed_tokens(
            prompt_text)  # [B, T_prompt_text_token, 896]
        next_fill_index = (int(prompt_speech_token.size(
            1) / self.mix_ratio[1]) + 1) * self.mix_ratio[1] - prompt_speech_token.size(1)  # 3
        for this_text in text:
            # 循环添加text token和audio token到lm_input。5个text token+15个audio token的比例。
            text_cache = torch.concat(
                [text_cache, self.llm.model.model.embed_tokens(this_text)], 1)  # 15+1(one token from this_text)
            # prompt_speech_token_emb not empty, try append to lm_input
            while prompt_speech_token_emb.size(1) != 0:
                if text_cache.size(1) >= self.mix_ratio[0]:
                    lm_input_text, lm_input_speech = text_cache[:, :self.mix_ratio[0]
                                                                ], prompt_speech_token_emb[:, :self.mix_ratio[1]]
                    logging.info("append {} text token {} speech token".format(
                        lm_input_text.size(1), lm_input_speech.size(1)))
                    lm_input = torch.concat(
                        [lm_input, lm_input_text, lm_input_speech], 1)  # sos_emb+5 text token+15 audio token
                    text_cache, prompt_speech_token_emb = text_cache[:,
                                                                     self.mix_ratio[0]:], prompt_speech_token_emb[:, self.mix_ratio[1]:]  # update to new
                else:
                    logging.info(
            ...
            if prompt_speech_token_emb.size(1) == 0:
                if (len(out_tokens) != 0 and out_tokens[-1] == self.fill_token) or (len(out_tokens) == 0 and lm_input.size(1) == 1):
                    logging.info(
                        "get fill token, need to append more text token")
                    if text_cache.size(1) >= self.mix_ratio[0]:
                        lm_input_text = text_cache[:, :self.mix_ratio[0]]
                        logging.info("append {} text token".format(
                            lm_input_text.size(1)))
                        if len(out_tokens) != 0 and out_tokens[-1] == self.fill_token:
                            lm_input = lm_input_text
                        else:
                            lm_input = torch.concat(
                                [lm_input, lm_input_text], 1)
                        ...
                        continue
                while True:
                    seq_len = lm_input.size(1) if cache is None else lm_input.size(
                        1) + cache[0][0].size(2)  # [B, 118, 896]
                    y_pred, cache = self.llm.forward_one_step(lm_input,
                                                              masks=torch.tril(torch.ones(
                                                                  (1, seq_len, seq_len), device=lm_input.device)).to(torch.bool),
                                                              cache=cache)
                    logp = self.llm_decoder(y_pred[:, -1]).log_softmax(-1)
                    # 判断是否添加一个fill_token
                    if next_fill_index != -1 and len(out_tokens) == next_fill_index:
                        top_ids = self.fill_token  # 添加一个fill_token
                        next_fill_index += (self.mix_ratio[1] + 1)
                    else:
                        top_ids = self.sampling_ids(logp.squeeze(
                            0), out_tokens, sampling, ignore_eos=True)
                    if top_ids == self.fill_token:
                        next_fill_index = len(
                            out_tokens) + self.mix_ratio[1] + 1  # 下次添加fill_token的位置
                        logging.info(
                            f"fill_token index {len(out_tokens)} next fill_token index {next_fill_index}")
                    out_tokens.append(top_ids)
        ...
        # 3. final decode
        lm_input = torch.concat(
            [lm_input, text_cache, task_id_emb], 1)  # [B, 154,896]
        while True:
            seq_len = lm_input.size(1) if cache is None else lm_input.size(
                1) + cache[0][0].size(2)
            y_pred, cache = self.llm.forward_one_step(lm_input,
                                                      masks=torch.tril(torch.ones(
                                                          (1, seq_len, seq_len), device=lm_input.device)).to(torch.bool),
                                                      cache=cache)  # [B, 154,896], dynamic_cache
            # [B, 6564],6564=self.speech_token_size+3, 3=sos_id+task_id+fill_id, id 6564=eos_id
            logp = self.llm_decoder(y_pred[:, -1]).log_softmax(-1)
            top_ids = self.sampling_ids(logp.squeeze(0),
                                        out_tokens,
                                        sampling,
                                        ignore_eos=False)  # one int token
            out_tokens.append(top_ids)
            if top_ids >= self.speech_token_size:
                if top_ids == self.eos_token:
                    break
