...
class CosyVoiceModel:
    ...
    def llm_job(self,
        ...
        with self.llm_context, torch.autocast("cuda", enabled=self.fp16 is True and hasattr(self.llm, "vllm") is False):
            if isinstance(text, Generator):
                ...
                token_generator = self.llm.inference_bistream(text=text,  # generator
                                                              prompt_text=prompt_text.to(
                                                                  self.device),  # [B, T_prompt_text_token=15]
                                                              prompt_text_len=torch.tensor(
                                                                  [prompt_text.size(1)], dtype=torch.int).to(self.device),
                                                              prompt_speech_token=llm_prompt_speech_token.to(
                                                                  self.device),  # [B, T_prompt_speech_token=87]
    ...
    def tts(self,
        ...
        if stream is True:
            ...
            yield {"tts_speech": this_tts_speech.cpu()}
        with self.lock:
            print(f"{self.tts_speech_token_dict=}")
...
class CosyVoice2Model(CosyVoiceModel):
    def __init__(self,
        ...
        self.llm_context = torch.cuda.stream(torch.cuda.Stream(
            self.device)) if torch.cuda.is_available() else nullcontext()
        self.lock = threading.Lock()
        # dict used to store session related variable
        self.tts_speech_token_dict = {}
        self.llm_end_dict = {}
    ...
    def token2wav(self, token,
        ...
        with torch.autocast("cuda", enabled=self.fp16):
            tts_mel, _ = self.flow.inference(
                ...
                token_len=torch.tensor(
                    [token.size(1)], dtype=torch.int32).to(self.device),
                prompt_token=prompt_token.to(self.device),
                prompt_token_len=torch.tensor(
                    [prompt_token.size(1)], dtype=torch.int32).to(self.device),
                prompt_feat=prompt_feat.to(self.device),
                prompt_feat_len=torch.tensor(
                    [prompt_feat.size(1)], dtype=torch.int32).to(self.device),
        ...
        if finalize is False:
            ...
            self.hift_cache_dict[uuid] = {"mel": tts_mel[:, :, -self.source_cache_len:],
                                          "source": tts_source[:, :, -self.source_cache_len:],
            ...
            if speed != 1.0:
                ...
                tts_mel = F.interpolate(tts_mel, size=int(
                    tts_mel.size(2) / speed), mode="linear")
            tts_speech, tts_source = self.hift.inference(
                speech_feat=tts_mel, cache_source=hift_cache_source)
            if self.hift_cache_dict[uuid] is not None:
                tts_speech = fade_in_out(
    ...
    def tts(self,
            # int, [B,T_text_token=32]
            text=torch.zeros(1, 0, dtype=torch.int32),
            flow_embedding=torch.zeros(0, 192),  # [B, 192]
            llm_embedding=torch.zeros(0, 192),  # [B, 192]
        ...
        if source_speech_token.size(1) == 0:
            ...
            p = threading.Thread(target=self.vc_job, args=(
                source_speech_token, this_uuid))
        p.start()
        if stream is True:
            token_offset = 0
            prompt_token_pad = int(np.ceil(flow_prompt_speech_token.size(
                1) / self.token_hop_len) * self.token_hop_len - flow_prompt_speech_token.size(1))
            while True:
                time.sleep(1e-1)
                this_token_hop_len = self.token_hop_len + \
                    prompt_token_pad if token_offset == 0 else self.token_hop_len
                if len(self.tts_speech_token_dict[this_uuid]) - token_offset >= this_token_hop_len + self.flow.pre_lookahead_len:
                    this_tts_speech_token = torch.tensor(
                        self.tts_speech_token_dict[this_uuid][:token_offset + this_token_hop_len + self.flow.pre_lookahead_lenb]).unsqueeze(0)
                    this_tts_speech = self.token2wav(token=this_tts_speech_token,
                                                     prompt_token=flow_prompt_speech_token,
                                                     prompt_feat=prompt_speech_feat,
                                                     embedding=flow_embedding,
                    ...
                    self.token_hop_len = min(
                        self.token_max_hop_len, self.token_hop_len * self.stream_scale_factor)
                    yield {"tts_speech": this_tts_speech.cpu()}
            ...
            this_tts_speech = self.token2wav(token=this_tts_speech_token,  # [B, T_generated_token=210]
                                             # [B, T_prompt_speech_token=87]
                                             prompt_token=flow_prompt_speech_token,
                                             # [B, T_prompt_mel=174,D=80]
                                             prompt_feat=prompt_speech_feat,
                                             # [B, 192]
...
class CosyVoice3Model(CosyVoice2Model):
    def __init__(self,
        ...
        self.device = torch.device(
            "cuda" if torch.cuda.is_available() else "cpu")
        self.llm = llm  # CosyVoice3LM
        self.flow = flow  # CausalMaskedDiffWithDiT
        self.hift = hift  # CausalHiFTGenerator
        self.fp16 = fp16  # false
        # NOTE must matching training static_chunk_size
        self.token_hop_len = 25
        # NOTE increase token_hop_len incrementally to avoid duplicate inference
        self.token_max_hop_len = 4 * self.token_hop_len  # 25*4
        self.stream_scale_factor = 2
        assert self.stream_scale_factor >= 1, "stream_scale_factor should be greater than 1, change it according to your actual rtf"
        # rtf and decoding related
        self.llm_context = torch.cuda.stream(torch.cuda.Stream(
        ...
        self.silent_tokens = [1, 2, 28, 29, 55,
                              248, 494, 2241, 2242, 2322, 2323]

    def token2wav(self,
                  # LLM generated semantic token, [B, T_generated_token=210]
                  token,
                  # prompt speech token, [B, T_prompt_speech_token=87]
                  prompt_token,
                  prompt_feat,  # prompt mel spec, [B, T_prompt_mel=174, D=80]
                  embedding,  # spk embedding, [B, 192]
                  token_offset,
                  uuid,
                  stream=False,
                  finalize=False,
                  speed=1.0):
        with torch.autocast("cuda", enabled=self.fp16):
            tts_mel, _ = self.flow.inference(token=token.to(self.device, dtype=torch.int32),  # LLM generated token, [B, T_generated_token=210]
                                             token_len=torch.tensor(
                                                 [token.size(1)], dtype=torch.int32, device=self.device),
                                             prompt_token=prompt_token.to(
                                                 self.device),  # prompt speech token, [B, T_prompt_speech_token=87]
                                             prompt_token_len=torch.tensor(
                                                 [prompt_token.size(1)], dtype=torch.int32, device=self.device),
                                             prompt_feat=prompt_feat.to(
                                                 self.device),  # prompt mel spec, [B, T_prompt_mel=174, D=80]
                                             prompt_feat_len=torch.tensor(
                                                 [prompt_feat.size(1)], dtype=torch.int32, device=self.device),
                                             embedding=embedding.to(
                                                 self.device),  # spk embedding, [B, 192]
                                             streaming=stream,
                                             finalize=finalize)  # [B, D=80, T]
            tts_mel = tts_mel[:, :, token_offset * self.flow.token_mel_ratio:]
            # append mel cache
            if self.hift_cache_dict[uuid] is not None:
            ...
            self.hift_cache_dict[uuid]["speech_offset"] += tts_speech.size(1)
        return tts_speech
