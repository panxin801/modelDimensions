...
class PosteriorEncoder2(nn.Module):
    def __init__(self,
                 in_channels,
                 out_channels,
                 hidden_channels,
        ...
        self.pre = nn.Conv1d(self.hidden_channels, self.hidden_channels, 1)
        self.enc = modules.WN(hidden_channels, kernel_size,
                              dilation_rate, n_layers, gin_channels=gin_channels)
        self.proj = nn.Conv1d(hidden_channels, out_channels * 2, 1)
        self.pre_emb = nn.Embedding(num_vocabs, self.in_channels)
        self.pre_conv = nn.ConvTranspose1d(
            self.in_channels, self.hidden_channels, kernel_size=5, padding=1, stride=2)
...
class SynthesizerTrn(nn.Module):
    ...
    def __init__(self,
        ...
        self.enc_p = PosteriorEncoder2(token_channels, inter_channels, hidden_channels,
                                       kernel_size, self.dilation_rate, n_layers, n_vocab, gin_channels=gin_channels)
        ...
        self.enc_q = PosteriorEncoder(spec_channels, inter_channels, hidden_channels,
                                      kernel_size, self.dilation_rate, n_layers, gin_channels=gin_channels)
        self.flow = ResidualCouplingBlock(
            inter_channels, hidden_channels, 5, 1, 4, gin_channels=gin_channels)
        self.ref_p = ReferenceEncoder(
            hid_C=hidden_channels, out_C=hidden_channels)

    def forward(self, token_seq, token_seq_lengths, audio, audio_lengths, reference_audio):

        # print('audio: ', audio.shape)
        token_seq, m_p, logs_p, token_seq_mask = self.enc_p(
            token_seq, token_seq_lengths)

        g = self.ref_p(reference_audio).unsqueeze(-1)
        z, m_q, logs_q, audio_mask = self.enc_q(audio, audio_lengths, g=g)
        z_p = self.flow(z, audio_mask, g=g)

        z_slice, ids_slice = commons.rand_slice_segments(
            z, audio_lengths, self.segment_size)
        o = self.dec(z_slice, g=g)

        return o, token_seq_lengths, ids_slice, token_seq_mask, audio_mask, (z, z_p, m_p, logs_p, m_q, logs_q)

    def infer(self, token_seq, token_seq_lengths, reference_audio, noise_scale=1, length_scale=1, noise_scale_w=1., max_len=None):
        token_seq, m_p, logs_p, token_seq_mask = self.enc_p(
            token_seq, token_seq_lengths)

        g = self.ref_p(reference_audio).unsqueeze(-1)

        z = self.flow(token_seq, token_seq_mask, g=g, reverse=True)

        o = self.dec((z * token_seq_mask)[:, :, :max_len], g=g)

        return o, token_seq_mask
