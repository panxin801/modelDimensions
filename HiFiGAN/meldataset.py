...
def get_dataset_filelist(a):
    with open(a.input_training_file, "rt", encoding="utf8") as fr:
        trainin_files = [os.path.join(a.input_wavs_dir, line.split(
            "|")[0] + ".wav") for line in fr.readlines() if len(line) > 0]
    with open(a.input_validation_file, "rt", encoding="utf8") as fr:
        validation_files = [os.path.join(a.input_wavs_dir, line.split(
            "|")[0] + ".wav") for line in fr.readlines() if len(line) > 0]
    return trainin_files, validation_files
        ...
        if not self.fine_tuning:
            if self.split:
                if audio.size(1) >= self.segment_size:
                    ...
                    audio = F.pad(0, (0, self.segment_size -
            ...
            if self.split:
                ...
                audio = F.pad(audio, (0, self.segment_size -
                              audio.size(1)), "constant")
