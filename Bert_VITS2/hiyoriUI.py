...
class Model:
    ...
    def __init__(self, config_path: str, model_path: str, device: str, language: str):
        ...
        self.version: str = (
            self.hps.version if hasattr(
                self.hps, "version") else latest_version
        )
...
if __name__ == "__main__":
    ...
    async def _voice(
            ...
            for _text in texts:
                ...
                for speaker_piece in speaker_pieces:
                    ...
                    for lang_piece in lang_pieces:
                        ...
                        t = lang_piece2[1]
                        text_language_speakers.append(
                            (t, lang.upper(), speaker))

        elif language == "AUTO":
            text_language_speakers: List[Tuple[str, str, str]] = [
                (final_text, language.upper().replace("JA", "JP"), speaker_name)
                for sub_list in [
                    split_by_language(_text, target_languages=[
                                      "zh", "ja", "en"])
                    for _text in texts
                    if _text != ""
                ]
        ...
        with BytesIO() as wavContent:
            ...
            response = Response(
                content=wavContent.getvalue(), media_type="audio/wav")
            return response
                ...
                if os.path.isdir(models_dir):
                    ...
                    for sub_file in sub_files:
                        relpath = os.path.realpath(
                            os.path.join(models_dir, sub_file))
                        if only_unloaded and relpath in loaded_models.path2ids.keys():
                            continue
                        if sub_file.endswith(".pth") and sub_file.startswith("G_"):
        ...
        if language is None:
            if len(examples["ZH"]) + len(examples["JP"]) + len(examples["EN"]) == 0:
                ...
                rand_num = random.randint(
                    0,
                    len(examples["ZH"]) + len(examples["JP"]) +
                    len(examples["EN"]) - 1,
                )
                # ZH
