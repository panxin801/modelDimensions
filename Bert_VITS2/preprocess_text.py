...
@click.command()
@click.option("--transcription-path", default=preprocess_text_config.transcription_path,
...
@click.option("--config-path", default=preprocess_text_config.config_path,
              type=click.Path(exists=True, file_okay=True, dir_okay=False))
...
def preprocess(
    ...
    with open(transcription_path, "rt", encoding="utf-8") as f:
        audioPaths = set()
        countSame = 0
        countNotFound = 0

        for line in f.readlines():
            utt, spk, language, text, phones, tones, word2ph = line.strip().split("|", 6)
    ...
    with open(train_path, "wt", encoding="utf8") as f:
        for line in train_list:
            f.write(line)

    with open(val_path, "wt", encoding="utf8") as f:
        for line in val_list:
            f.write(line)

    json_config = json.load(open(config_path, encoding="utf8"))
    ...
    with open(config_path, "wt", encoding="utf8") as f:
