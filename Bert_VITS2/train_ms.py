...
def train_and_evaluate(
    ...
    for batch_idx, (
        ...
        y, y_lengths = y.cuda(local_rank, non_blocking=True), y_lengths.cuda(
            local_rank, non_blocking=True)
        speakers, tone, language = speakers.cuda(local_rank, non_blocking=True), tone.cuda(
            local_rank, non_blocking=True), language.cuda(local_rank, non_blocking=True)
        bert, ja_bert, en_bert = bert.cuda(local_rank, non_blocking=True), ja_bert.cuda(
            local_rank, non_blocking=True), en_bert.cuda(local_rank, non_blocking=True)
        ...
        with autocast(enabled=hps.train.bf16_run, dtype=torch.bfloat16):
            loss_slm = wl.discriminator(
                y.detach().squeeze(), y_hat.detach().squeeze()).mean()
        ...
        with autocast(enabled=hps.train.bf16_run, dtype=torch.bfloat16):
            ...
            if not net_dur_disc is None:
                _, y_dur_hat_g = net_dur_disc(hidden_x,
                                              x_mask,
                                              logw_,
                                              logw,
                                              g)
