...
class MyWriter(SummaryWriter):
    ...
    def log_fig_audio(self, real, fake, spec_fake, spec_real, idx, step):
        if idx == 0:
            spec_fake = librosa.amplitude_to_db(
                spec_fake, ref=np.max, top_db=80.)
            spec_real = librosa.amplitude_to_db(
                spec_real, ref=np.max, top_db=80.)
            self.add_image(
                f'spec_fake/{step}', plot_spectrogram_to_numpy(spec_fake), step)
            self.add_image(f'wave_fake/{step}',
                           plot_waveform_to_numpy(fake), step)
            self.add_image(
                f'spec_real/{step}', plot_spectrogram_to_numpy(spec_real), step)
            self.add_image(f'wave_real/{step}',
                           plot_waveform_to_numpy(real), step)
