...
from vits_extend.dataloader import (
    create_dataloader_train, create_dataloader_eval)
...
def train(rank, args, checkpoint_path, hp, hp_str):
    if args.num_gpus > 1:
        dist.init_process_group(
            hp.dist_config.dist_backend,
            hp.dist_config.dist_url,
    ...
    optim_g = optim.AdamW(model_g.parameters(
    ), lr=hp.train.learning_rate, betas=hp.train.betas, eps=hp.train.eps)
    optim_d = optim.AdamW(model_d.parameters(), lr=(
        hp.train.learning_rate / hp.train.accum_step), betas=hp.train.betas, eps=hp.train.eps)
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(os.path.join(
                    log_dir, '%s-%d.log' % (args.name, time.time()))),
    ...
    for epoch in range(init_epoch, hp.train.epochs):
        ...
        if rank == 0 and epoch % hp.log.eval_interval == 0:
            with torch.inference_mode():
                validate(hp, args, model_g, model_d,
                         valloader, stft, writer, step, device)
        ...
        for ppg, ppg_l, vec, pit, spk, spec, spec_l, audio, audio_l in loader:
            ...
            if ((step + 1) % hp.train.accum_step == 0) or (step + 1 == len(loader)):
                ...
                commons.clip_grad_value_(model_g.parameters(), None)
                # update model
                optim_g.step()
                optim_g.zero_grad()
            ...
            loss_d.backward()
            commons.clip_grad_value_(model_d.parameters(), None)
            optim_d.step()
        ...
        if rank == 0:
            def clean_checkpoints(path_to_models=f'{pth_dir}', n_ckpts_to_keep=hp.log.keep_ckpts, sort_by_time=True):
                ...
                ckpts_files = [f for f in os.listdir(
                    path_to_models) if os.path.isfile(os.path.join(path_to_models, f))]
                name_key = (lambda _f: int(re.compile(
                    f'{args.name}_(\d+)\.pt').match(_f).group(1)))
                time_key = (lambda _f: os.path.getmtime(
                ...
                if n_ckpts_to_keep == 0:
                    ...
                    to_del = [os.path.join(path_to_models, fn) for fn in x_sorted(
                        f'{args.name}')[:-n_ckpts_to_keep]]
            ...
            if keep_ckpts > 0:
                clean_checkpoints(
                    path_to_models=f'{pth_dir}', n_ckpts_to_keep=hp.log.keep_ckpts, sort_by_time=True)
