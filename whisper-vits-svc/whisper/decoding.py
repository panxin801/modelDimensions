...
def detect_language(model: "Whisper", mel: torch.Tensor, tokenizer: Tokenizer = None) -> Tuple[torch.Tensor, List[dict]]:
    """
...
class SequenceRanker:
    def rank(self, tokens: List[List[torch.Tensor]], sum_logprobs: List[List[float]]) -> List[int]:
        """
...
class TokenDecoder:
    ...
    def update(self, tokens: torch.Tensor, logits: torch.Tensor, sum_logprobs: torch.Tensor) -> Tuple[torch.Tensor, bool]:
...
class DecodingTask:
    ...
    def _main_loop(self, audio_features: torch.Tensor, tokens: torch.Tensor):
        ...
        sum_logprobs: torch.Tensor = torch.zeros(
    ...
    def run(self, mel: torch.Tensor) -> List[DecodingResult]:
        ...
        tokens: torch.Tensor = torch.tensor(
            [self.initial_tokens]).repeat(n_audio, 1)
...
def decode(model: "Whisper", mel: torch.Tensor, options: DecodingOptions = DecodingOptions()) -> Union[DecodingResult, List[DecodingResult]]:
    ...
    mel: torch.torch.Tensor, shape = (80, 3000) or (*, 80, 3000)
