...
class SpeakerAdapter(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.spekaer_dim = speaker_dim
        self.adapter_dim = adapter_dim
        self.epsilon = epsilon
        self.W_scale = nn.Linear(speaker_dim, adapter_dim)
        self.W_bias = nn.Linear(speaker_dim, adapter_dim)
    ...
    def __init__(self, hp):
        ...
        for i, (u, k) in enumerate(zip(hp.gen.upsample_rates, hp.gen.upsample_kernel_sizes)):
            ...
            if i + 1 < len(hp.gen.upsample_rates):
                ...
                self.noise_convs.append(
                    nn.Conv1d(1, hp.gen.upsample_initial_channel //
                              (2 ** (i + 1)), kernel_size=1)
                )
