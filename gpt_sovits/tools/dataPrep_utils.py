        ...
        if is_train:
            for file, status in zip(file_list, files_status):
                if status:
                    ...
                    warnings.warn(f"{file} 不存在")
            warnings.warn(i18n("以下文件或文件夹不存在"))
            return False
        elif is_dataset_processing:
            if files_status[0]:
                return True
            elif not files_status[0]:
                warnings.warn(file_list[0])
            elif not files_status[1] and file_list[1]:
                warnings.warn(file_list[1])
            warnings.warn(i18n("以下文件或文件夹不存在"))
            return False
        else:
            if file_list[0]:
...
def check_details(path_list=None, is_train=False, is_dataset_processing=False):
    if is_dataset_processing:
        ...
        if not list_path.endswith(".list"):
            warnings.warn(i18n("请填入正确的List路径"))
            return
        if audio_path:
            if not os.path.isdir(audio_path):
                warnings.warn(i18n("请填入正确的音频文件夹路径"))
                return
    ...
    if is_train:
        ...
        with open(phone_path, "r", encoding="utf-8") as f:
            if f.read(1):
                ...
                warnings.warn(i18n("缺少音素数据集"))
        if os.listdir(hubert_path):
            ...
        else:
            warnings.warn(i18n("缺少Hubert数据集"))
        if os.listdir(wav_path):
            ...
        else:
            warnings.warn(i18n("缺少音频数据集"))
        df = pd.read_csv(semantic_path, delimiter="\t", encoding="utf-8")
        if len(df) >= 1:
            ...
        else:
            warnings.warn(i18n("缺少语义数据集"))
