...
class DiT(nn.Module):
    ...
    def forward(  # x, prompt_x, x_lens, t, style,cond
        ...
        text_embed = self.text_embed(
            text, seq_len, drop_text=drop_text)  # need to change
        x = self.input_embed(x, cond, text_embed,
                             drop_audio_cond=drop_audio_cond)

        rope = self.rotary_embed.forward_from_seq_len(seq_len)

        if not self.long_skip_connection is None:
            residual = x

        for block in self.transformer_blocks:
            if use_grad_ckpt:
                x = checkpoint(self.ckpt_wrapper(block), x, t,
                               mask, rope, use_reentrant=False)
            else:
