...
def get_bigvgan_mel_spectrogram(
    ...
    if key not in mel_basis_cache:
        mel = librosa_mel_fn(sr=target_sample_rate, n_fft=n_fft,
                             n_mels=n_mel_channels, fmin=fmin, fmax=fmax)
        mel_basis_cache[key] = torch.from_numpy(mel).float().to(
            device)  # TODO: why they need .float()?
    ...
    waveform = torch.nn.functional.pad(waveform.unsqueeze(
        1), (padding, padding), mode="reflect").squeeze(1)
...
def get_pos_embed_indices(start, length, max_pos, scale=1.0):
    ...
    pos = (
        start.unsqueeze(1)
        + (torch.arange(length, device=start.device,
           dtype=torch.float32).unsqueeze(0) * scale.unsqueeze(1)).long()
    )
...
class FeedForward(nn.Module):
    def __init__(self, dim, dim_out=None, mult=4, dropout=0.0, approximate: str = "none"):
        ...
        self.ff = nn.Sequential(project_in, nn.Dropout(
...
class MMDiTBlock(nn.Module):
    ...
    def __init__(self, dim, heads, dim_head, ff_mult=4, dropout=0.1, context_pre_only=False):
        ...
        self.attn_norm_c = AdaLayerNormZero_Final(
            dim) if context_pre_only else AdaLayerNormZero(dim)
    ...
    def forward(self, x, c, t, mask=None, rope=None, c_rope=None):
        ...
        if self.context_pre_only:
            ...
            norm_c = self.ff_norm_c(
                c) * (1 + c_scale_mlp[:, None]) + c_shift_mlp[:, None]
        ...
        norm_x = self.ff_norm_x(
            x) * (1 + x_scale_mlp[:, None]) + x_shift_mlp[:, None]
