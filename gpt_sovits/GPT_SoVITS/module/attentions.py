...
class Decoder(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        for i in range(self.n_layers):
            ...
            self.cross_attn_layers.append(MultiHeadAttention(hidden_channels,
                                                             hidden_channels,
                                                             n_heads,
                                                             p_dropout=p_dropout,)
                                          )
    ...
    def forward(self, x, x_mask, h, h_mask):
        ...
        for i in range(self.n_layers):
            ...
            y = self.drop(y)
            y = self.norm_layers_0[i][x + y]

            y = self.cross_attn_layers[i](x, h, cross_attn_mask)
            y = self.drop(y)
            x = self.norm_layers_1[i](x + y)
...
class Depthwise_Separable_Conv1D(nn.Module):
    ...
    def weight_norm(self):
        self.depth_conv = nn.utils.weight_norm(self.depth_conv, name="weight")
        self.point_conv = nn.utils.weight_norm(self.point_conv, name="weight")

    def remove_weight_norm(self):
        self.depth_conv = nn.utils.remove_weight_norm(
            self.depth_conv, naem="weight")
        self.point_conv = nn.utils.remove_weight_norm(
            self.point_conv, naem="weight")
...
class Depthwise_Separable_TransposeConv1D(nn.Module):
    ...
    def remove_weight_norm(self):
        self.depth_conv = nn.utils.remove_weight_norm(
            self.depth_conv, name="weight")
        self.point_conv = nn.utils.remove_weight_norm(
            self.point_conv, name="weight")


def weight_norm_modules(module, name="weight", dim=0):
    if isinstance(module, (Depthwise_Separable_Conv1D, Depthwise_Separable_TransposeConv1D)):
        module.weight_norm()
        return module
    else:
        return nn.utils.weight_norm(module, name, dim)
...
def remove_weight_norm_modules(module, name="weight"):
    if isinstance(module, (Depthwise_Separable_Conv1D, Depthwise_Separable_TransposeConv1D)):
        ...
        return nn.utils.remove_weight_norm(module, name)
