        ...
        if not test is None:
            if test == 0:
                x = self.cross_attention(
                    ssl_enc * ssl_mask, text_enc * text_mask, attn_mask) + ssl_enc + ge
            elif test == 1:
                x = ssl_enc + ge
            elif test == 2:
                x = self.cross_attention(
                    ssl_enc * 0 * ssl_mask, text_enc * text_mask, attn_mask) + ge
            else:
                raise ValueError("test should be 0, 1 or 2")
        else:
            x = self.cross_attention(
                ssl_enc * ssl_mask, text_enc * text_mask, attn_mask) + ssl_enc + ge
...
if __name__ == "__main__":
    ...
    ref_mel = torch.randn(3, 192, 30)
    ref_mask = torch.ones(3, 1, 30)
    model = MRTE()
    out = model(content_enc, content_mask, ref_mel, ref_mask, 0)
    print(out.shape)
