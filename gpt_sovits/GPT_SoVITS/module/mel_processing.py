...
def spectrogram_torch(y, n_fft, sampling_rate, hop_size, win_size, center=False):
    ...
    key = "%s-%s-%s-%s-%s" % (dtype_device, n_fft,
                              sampling_rate, hop_size, win_size)
    # if wnsize_dtype_device not in hann_window:
    if key not in hann_window:
        # hann_window[wnsize_dtype_device] = torch.hann_window(win_size).to(dtype=y.dtype, device=y.device)
        hann_window[key] = torch.hann_window(
...
def spec_to_mel_torch(spec, n_fft, num_mels, sampling_rate, fmin, fmax):
    ...
    key = "%s-%s-%s-%s-%s-%s" % (dtype_device, n_fft,
    ...
    if key not in mel_basis:
        ...
        mel_basis[key] = torch.from_numpy(mel).to(
...
def mel_spectrogram_torch(y, n_fft, num_mels, sampling_rate, hop_size, win_size, fmin, fmax, center=False):
    ...
    fmax_dtype_device = "%s-%s-%s-%s-%s-%s-%s-%s" % (
        dtype_device, n_fft, num_mels, sampling_rate, hop_size, win_size, fmin, fmax)
    # wnsize_dtype_device = str(win_size) + '_' + dtype_device
