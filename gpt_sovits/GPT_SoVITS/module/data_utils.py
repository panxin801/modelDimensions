...
class TextAudioSpeakerLoader(Data.Dataset):
    """
    ...
    def random_slice(self, ssl, wav, mel):
        assert abs(ssl.shape[-1] - wav.shape[-1] //
                   self.hop_length) < 3, ("first", ssl.shape, wav.shape)
...
class TextAudioSpeakerCollate:
    ...
    def __call__(self, batch):
        ...
        _, ids_sorted_decreasing = torch.sort(torch.LongTensor(
            [x[1].size(1) for x in batch]), dim=0, descending=True)
...
class TextAudioSpeakerCollateV3:
    ...
    def __call__(self, batch):
        ...
        mel_padded = torch.FloatTensor(
            len(batch), batch[0][2].size(0), max_mel_len)
        ssl_padded = torch.FloatTensor(
            len(batch), batch[0][0].size(1), max_ssl_len)
        text_padded = torch.LongTensor(len(batch), max_text_len)
        # wav_padded = torch.FloatTensor(len(batch), 1, max_wav_len)
...
class TextAudioSpeakerLoaderV4(torch.utils.data.Dataset):
    ...
    def get_audio(self, filename):
        ...
        spec = torch.squeeze(spec, 0)
        spec1 = spectrogram_torch(
            audio_norm, 1280, 32000, 320, 1280, center=False)
...
class TextAudioSpeakerCollateV3b:
    ...
    def __call__(self, batch):
        ...
        spec_padded = torch.FloatTensor(
            len(batch), batch[0][1].size(0), max_spec_len)
        mel_padded = torch.FloatTensor(
            len(batch), batch[0][3].size(0), max_mel_len)
        ssl_padded = torch.FloatTensor(
            len(batch), batch[0][0].size(1), max_ssl_len)
        text_padded = torch.LongTensor(len(batch), max_text_len)
        wav_padded = torch.FloatTensor(len(batch), 1, max_wav_len)
