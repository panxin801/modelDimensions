...
class DistributedBucketSampler(Sampler[T_co]):
    ...
    def __init__(
        ...
        if rank >= num_replicas or rank < 0:
            raise ValueError("Invalid rank {}, rank should be in the interval [0, {}]".format(
                rank, num_replicas - 1))
    ...
    def __iter__(self) -> Iterator[T_co]:
        if self.shuffle:
            ...
            batches = [shuffled_bucket[b * grouped_batch_size: (
                b + 1) * grouped_batch_size] for b in range(n_batch)]
            shuffle(batches)
