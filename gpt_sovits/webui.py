...
def open1Ba(
    ...
    if p_train_SoVITS == None:
        ...
        yield (
            ...
            {"__type__": "update", "visible": True},
            {"__type__": "update",
                "visible": False}, SoVITS_dropdown_update, GPT_dropdown_update
        )
    else:
        yield (
    ...
    if ps1abc == []:
        ...
        try:
            ...
            if os.path.exists(path_semantic) == False or (
                os.path.exists(path_semantic) == True and os.path.getsize(
                    path_semantic) < 31
            ):
                config = {
                    "inp_text": inp_text,
...
def switch_version(version_):
    ...
    return (
        ...
        {"__type__": "update", "value": pretrained_sovits_name[int(
            version[-1]) - 1].replace("s2G", "s2D")},
        {"__type__": "update",
        ...
        {"__type__": "update",
            "value": pretrained_sovits_name[int(version[-1]) - 1]},
        {"__type__": "update", "value": default_batch_size,
            "maximum": default_max_batch_size},
        {"__type__": "update", "value": default_sovits_epoch,
            "maximum": max_sovits_epoch},
        {"__type__": "update", "value": default_sovits_save_every_epoch,
                ...
                with gr.Column(scale=3):
                    with gr.Row():
                        uvr5_info = gr.Textbox(
                            label=process_info(process_name_uvr5, "info"))
                open_uvr5 = gr.Button(value=process_info(
                    process_name_uvr5, "open"), variant="primary", visible=True)
                close_uvr5 = gr.Button(value=process_info(
                    process_name_uvr5, "close"), variant="primary", visible=False)
            ...
            with gr.Row():
                with gr.Column(scale=3):
                    with gr.Row():
                        ...
                        slice_opt_root = gr.Textbox(label=i18n(
                            "切分后的子音频的输出根目录"), value="output/slicer_opt")
                    with gr.Row():
                        threshold = gr.Textbox(label=i18n(
                            "threshold:音量小于这个值视作静音的备选切割点"), value="-34")
                        min_length = gr.Textbox(
                            label=i18n(
                                "min_length:每段最小多长，如果第一段太短一直和后面段连起来直到超过这个值"),
                            value="4000",
                        )
                        min_interval = gr.Textbox(label=i18n(
                            "min_interval:最短切割间隔"), value="300")
                        hop_size = gr.Textbox(
                            label=i18n(
                                "hop_size:怎么算音量曲线，越小精度越大计算量越高（不是精度越大效果越好）"),
                            value="10",
                        )
                        max_sil_kept = gr.Textbox(label=i18n(
                            "max_sil_kept:切完后静音最多留多长"), value="500")
                    with gr.Row():
                        _max = gr.Slider(
                            minimum=0,
                            maximum=1,
                    ...
                    with gr.Row():
                        ...
                        slicer_info = gr.Textbox(
                            label=process_info(process_name_slice, "info"))
                ...
                with gr.Column(scale=3):
                    with gr.Row():
                        denoise_input_dir = gr.Textbox(
                            label=i18n("输入文件夹路径"), value="")
                        denoise_output_dir = gr.Textbox(label=i18n(
                            "输出文件夹路径"), value="output/denoise_opt")
                    with gr.Row():
                        denoise_info = gr.Textbox(
                            label=process_info(process_name_denoise, "info"))
            ...
            with gr.Row():
                with gr.Column(scale=3):
                    with gr.Row():
                        ...
                        asr_opt_dir = gr.Textbox(label=i18n(
                            "输出文件夹路径"), value="output/asr_opt", interactive=True)
                    with gr.Row():
                    ...
                    with gr.Row():
                        asr_info = gr.Textbox(
                            label=process_info(process_name_asr, "info"))
                ...
                asr_model.change(change_precision_choices, [
                                 asr_model], [asr_precision])
            ...
            with gr.Row():
                with gr.Column(scale=3):
                    with gr.Row():
                        ...
                        label_info = gr.Textbox(
                            label=process_info(process_name_subfix, "info"))
                open_label = gr.Button(value=process_info(
                    process_name_subfix, "open"), variant="primary", visible=True)
                close_label = gr.Button(
                    value=process_info(process_name_subfix, "close"), variant="primary", visible=False
                )

            open_label.click(change_label, [path_list], [
                             label_info, open_label, close_label])
            close_label.click(change_label, [path_list], [
                              label_info, open_label, close_label])
            open_uvr5.click(change_uvr5, [], [
                            uvr5_info, open_uvr5, close_uvr5])
            close_uvr5.click(change_uvr5, [], [
                             uvr5_info, open_uvr5, close_uvr5])

        with gr.TabItem(i18n("1-GPT-SoVITS-TTS")):
            with gr.Row():
                with gr.Row():
                    exp_name = gr.Textbox(label=i18n(
                        "*实验/模型名"), value="xxx", interactive=True)
                    gpu_info = gr.Textbox(label=i18n(
                        "显卡信息"), value=gpu_info, visible=True, interactive=False)
                    version_checkbox = gr.Radio(label=i18n("版本"), value=version, choices=[
                                                "v1", "v2", "v4"])  # , "v3"
                with gr.Row():
                    ...
                    pretrained_s2D = gr.Textbox(
                        ...
                        value=pretrained_sovits_name[int(
                            version[-1]) - 1].replace("s2G", "s2D"),
                        interactive=True,
                        lines=2,
                        max_lines=3,
            ...
            with gr.TabItem("1A-" + i18n("训练集格式化工具")):
                ...
                with gr.Row():
                    ...
                    with gr.Row():
                        info1a = gr.Textbox(
                            label=process_info(process_name_1a, "info"))
                ...
                with gr.Row():
                    ...
                    with gr.Row():
                        info1b = gr.Textbox(
                            label=process_info(process_name_1b, "info"))
                ...
                with gr.Row():
                    ...
                    with gr.Row():
                        info1c = gr.Textbox(
                            label=process_info(process_name_1c, "info"))
                ...
                with gr.Row():
                    ...
                    with gr.Row():
                        info1abc = gr.Textbox(
                            label=process_info(process_name_1abc, "info"))

            pretrained_s2G.change(sync, [pretrained_s2G], [pretrained_s2G_])
            open_asr_button.click(
                open_asr,
                [asr_inp_dir, asr_opt_dir, asr_model,
                    asr_size, asr_lang, asr_precision],
                [asr_info, open_asr_button, close_asr_button,
                    path_list, inp_text, inp_wav_dir],
            )
            ...
            open_denoise_button.click(
                ...
                [denoise_info, open_denoise_button,
                    close_denoise_button, asr_inp_dir, inp_wav_dir],
            )
            close_denoise_button.click(
                close_denoise, [], [denoise_info, open_denoise_button, close_denoise_button])

            button1a_open.click(
                open1a,
                [inp_text, inp_wav_dir, exp_name,
                    gpu_numbers1a, bert_pretrained_dir],
                [info1a, button1a_open, button1a_close],
            )
            button1a_close.click(
                close1a, [], [info1a, button1a_open, button1a_close])
            button1b_open.click(
                open1b,
                [inp_text, inp_wav_dir, exp_name,
                    gpu_numbers1Ba, cnhubert_base_dir],
                [info1b, button1b_open, button1b_close],
            )
            button1b_close.click(
                close1b, [], [info1b, button1b_open, button1b_close])
            button1c_open.click(
                open1c, [inp_text, exp_name, gpu_numbers1c, pretrained_s2G], [
                    info1c, button1c_open, button1c_close]
            )
            button1c_close.click(
                close1c, [], [info1c, button1c_open, button1c_close])
            ...
            with gr.TabItem("1B-" + i18n("微调训练")):
                gr.Markdown(value="1Ba-" +
                            i18n("SoVITS 训练: 模型权重文件在 SoVITS_weights/"))
                with gr.Row():
                    with gr.Column():
                        with gr.Row():
                ...
                with gr.Row():
                    ...
                    with gr.Row():
                        info1Ba = gr.Textbox(label=process_info(
                            process_name_sovits, "info"))
                gr.Markdown(value="1Bb-" +
                            i18n("GPT 训练: 模型权重文件在 GPT_weights/"))
                with gr.Row():
                    with gr.Column():
                        with gr.Row():
                ...
                with gr.Row():
                    ...
                    with gr.Row():
                        info1Bb = gr.Textbox(
                            label=process_info(process_name_gpt, "info"))
            ...
            with gr.TabItem("1C-" + i18n("推理")):
                ...
                with gr.Row():
                    ...
                    with gr.Row():
                        gpu_number_1C = gr.Textbox(label=i18n(
                            "GPU卡号,只能填1个整数"), value=gpus, interactive=True)
                ...
                with gr.Row():
                    ...
                    with gr.Row():
                        tts_info = gr.Textbox(
                            label=process_info(process_name_tts, "info"))
                    open_tts.click(
